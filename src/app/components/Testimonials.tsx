...
export function Testimonials() {
  ...
  return (
    <section className="py-24 bg-[#F2F8F9] overflow-hidden">
      <div className="max-w-[1440px] mx-auto px-6 md:px-10">
        <div className="text-center mb-16">
          ...
          <motion.h2
            ...
            viewport={{ once: true }}
            className="font-['Inter'] font-bold text-4xl md:text-5xl text-[#2C3E50] mb-6 tracking-tight"
        ...
        <div className="relative mb-24 -mx-6 md:-mx-10 px-6 md:px-10">
          <div className="overflow-hidden">
            ...
            >
              {carouselItems.map((item, idx) => (
                <div
                  key={idx}
                  className="w-[480px] rounded-[12px] p-[40px] flex flex-col justify-between gap-[32px] shrink-0 shadow-sm border border-[rgba(0,0,0,0.06)] bg-white"
                >
                  {/* Quote mark */}
                  <svg
                    width="32"
                  ...
                  {/* Text */}
                  <p
                    className="font-['Inter'] font-normal text-[18px] md:text-[20px] leading-[1.5] tracking-tight flex-grow text-[#2C3E50]"
                  >
                    "{item.quote}"
                  </p>

                  {/* Author */}
                  <div className="flex items-center gap-[16px] pt-[24px] border-t border-[rgba(0,0,0,0.08)]">
                    <div className="w-[56px] h-[56px] rounded-full overflow-hidden shrink-0 border-2 border-white shadow-sm">
                      <ImageWithFallback
                        src={item.image}
                        alt={item.name}
                    ...
                    <div className="flex flex-col">
                      <span className="font-['Inter'] font-bold text-[18px] text-[#2C3E50]">
                        {item.name}
                      </span>
                      <span className="font-['Inter'] text-[14px] text-[#636363]">
          ...
          <div className="absolute inset-y-0 left-0 w-24 md:w-48 bg-gradient-to-r from-[#F2F8F9] to-transparent z-10 pointer-events-none" />
          <div className="absolute inset-y-0 right-0 w-24 md:w-48 bg-gradient-to-l from-[#F2F8F9] to-transparent z-10 pointer-events-none" />
        </div>
        ...
        <motion.div
          ...
          className="border border-[rgba(0,0,0,0.08)] rounded-[16px] bg-white grid grid-cols-4"
        >
          {STATS.map((stat, idx) => (
            <div
              key={stat.label}
              className={`flex flex-col gap-[10px] px-[48px] py-[44px] ${idx < STATS.length - 1 ? "border-r border-[rgba(0,0,0,0.08)]" : ""}`}
            >
              <span
                className="font-['Inter'] font-bold text-[#00838F] leading-none"
                style={{ fontSize: "44px", letterSpacing: "-2px" }}
              >
                {stat.value}
              </span>
              <span
                className="font-['Inter'] font-normal text-[#636363] leading-[1.4]"
