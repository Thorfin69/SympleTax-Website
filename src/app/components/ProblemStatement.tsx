        ...
        <div className="flex items-end justify-between gap-12 mb-[60px]">
          <div className="flex flex-col gap-[22px]">
            <div className="flex items-center gap-[10px]">
              <div className="bg-[#2c3e50] h-px w-[35px] shrink-0" />
              <span className="font-['Inter'] font-medium text-[14px] uppercase tracking-[0.05em] text-[#2c3e50]">
                Why Choose Us
              </span>
            </div>
            <h2
          ...
          <div className="flex flex-col items-end gap-5 shrink-0 max-w-[320px]">
            <p
              className="font-['Inter'] font-medium text-[#2C3E50] leading-[1.65] text-right"
            ...
            <Link
              to="/services"
              className="bg-[#2C3E50] text-white flex items-center gap-4 px-8 py-4 rounded-full transition-all hover:bg-[#00838F] whitespace-nowrap"
            >
          ...
          <div className="flex-1 flex flex-col justify-center">
            <div className="border-t border-[rgba(0,0,0,0.07)]">
              {PROBLEMS.map((item) => {
                ...
                return (
                  <div key={item.id} className="border-b border-[rgba(0,0,0,0.07)]">
                    <button
                      onClick={() => setActiveId(item.id)}
                      className="flex items-center gap-[24px] w-full text-left py-[28px] group"
                    >
                      <span className="font-['Inter'] font-medium text-[24px] text-[#2C3E50] shrink-0 w-[50px]">
                        [{item.id}]
                      </span>
                      <span className={`font-['Inter'] font-medium text-[24px] transition-colors ${isActive ? "text-[#2C3E50]" : "text-[#2C3E50] group-hover:text-[#00838F]"}`}>
                        {item.title}
                      </span>
                    </button>
                      ...
                      {isActive && (
                        ...
                        >
                          <div className="pb-[40px] pl-[74px]">
                            <p className="font-['Inter'] font-normal text-[16px] text-[#2C3E50]/70 leading-[1.6] mb-6">
                              {item.description}
                            </p>
                            <motion.button
                              whileHover="hover"
                            ...
                            >
                              <span className="font-['Inter'] font-medium text-[15px] text-[#00838F]">
                                Learn More
                              </span>
                              <motion.div
                                variants={{ hover: { x: 4 } }}
                              ...
                              >
                                <ArrowUpRight className="text-[#00838F] size-[18px]" />
                              </motion.div>
                            </motion.button>
                          </div>
                        </motion.div>
