  ...
  return (
    <section className="bg-white py-[120px]">
      <div className="max-w-[1330px] mx-auto px-[70px]">
        ...
        <div className="flex items-end justify-between gap-12 mb-[40px]">
          <div className="flex flex-col gap-[14px]">
            <div className="flex items-center gap-[10px]">
              ...
              <span className="font-['Inter'] font-medium uppercase text-[#0f172a]" style={{ fontSize: "14px", letterSpacing: "0.05em" }}>
                Two Paths to Relief
              </span>
            </div>
            <h2 className="font-['Inter'] font-bold text-[#0f172a] leading-[1.08] whitespace-nowrap" style={{ fontSize: "clamp(28px, 3vw, 42px)", letterSpacing: "-1.5px" }}>
        ...
        <div className="flex items-center gap-8 mb-[32px] border-b border-slate-200">
          {(Object.keys(TRACKS) as TrackKey[]).map((key) => (
            <button
              ...
              className={`relative pb-4 font-['Inter'] font-semibold text-[16px] transition-colors duration-200 ${
                activeTrack === key ? "text-[#0f172a]" : "text-slate-400 hover:text-slate-600"
              }`}
            >
              {TRACKS[key].label}
              {activeTrack === key && (
                <motion.div
                  ...
                  className="absolute bottom-0 left-0 right-0 h-[2px] bg-[#00A4A4]"
        ...
        <div className="relative rounded-[20px] overflow-hidden" style={{ height: "580px" }}>
          ...
          {/* Dark overlay */}
          <div className="absolute inset-0 bg-[#0e7490]/60" />
                ...
                {track.items.map((it, i) => (
                  <button
                    ...
                    className={`px-4 py-2 rounded-full font-['Inter'] font-medium text-[13px] border transition-all duration-200 whitespace-nowrap ${
                      i === activeIdx
                        ? "bg-white text-[#0e7490] border-white shadow-[0_4px_16px_rgba(0,0,0,0.15)]"
                        : "bg-white/10 text-white/70 border-white/30 hover:bg-white/20 hover:border-white/60 hover:text-white"
                    }`}
                  >
                    {it.title}
                  </button>
